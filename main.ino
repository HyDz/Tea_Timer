 ...
 * Author: HyDz
 * 
 * A simple Tea Timer with Nokia 5510 screen and a speaker.
 * 
 * Use 
 * Tone librarie
...
//cablage de l'ecran lcd type nokia 5110
/*
LED -> 3v3 (pas cablé dans mon cas)
SCLK -> D3
MOSI -> D4
D/C -> D5
RST -> D6
...
//Pins des différents composants
const int PBoutPin = 10; // Bouton Plus
const int MBoutPin = 2; // Bouton Moins
const int SBoutPin = 8; // Bouton start/stop/reset
...
const int increment = 30; // Nombre de secondes qui s'incrémentent avec les boutons +/-
const int Bout_Sensitivity = 500;
...
{
  pinMode(PBoutPin, INPUT);
  digitalWrite(PBoutPin, LOW);
  
pinMode(MBoutPin, INPUT);
  digitalWrite(MBoutPin, LOW);
  
  pinMode(SBoutPin, INPUT);
  digitalWrite(SBoutPin, LOW);
  
  
}
//--------------------------------------

void _lowlevel_ReadBout(int& PBoutState, int& MBoutState, int& SBoutState)
{
  PBoutState = digitalRead(PBoutPin);
  MBoutState = digitalRead(MBoutPin);
  SBoutState = digitalRead(SBoutPin);
}

//---------------------------------------

void SetTimer() // Lit l'etat des boutons et ajuste l'affichage en fonction
{
Affich();
 int PBout, MBout, SBout; 
 seconds = constrain (seconds, -1, 60);
_ResetPins();
_lowlevel_ReadBout(PBout, MBout, SBout);
int PBout2, MBout2, SBout2;
do
{
  _lowlevel_ReadBout(PBout2, MBout2, SBout2);
}
while((PBout2 == PBout) && (MBout2 == MBout) && (SBout2 == SBout));
if (PBout2 != PBout) // si le bouton + est appuyé
{
  delay(Bout_Sensitivity);
  seconds= seconds + increment;
}
if (MBout2 != MBout) // si le bouton - est appuyé
{
delay(Bout_Sensitivity);
  if (seconds == 0); // si on est déjà à zéro secondes
{
  if (minutes > 0) // si il reste des minutes à retirer
  {
    minutes --;
...
{
seconds = seconds - increment; //si les secondes ne sont pas à zéro
}
}
}
if (seconds>59) // secondes arrivées à 60
{
  minutes++; // ajoute une minute
  seconds -= 60; // remet les secondes à zero
}

if (SBout2 != SBout) // Si le bouton start est appuyé
{
  delay(Bout_Sensitivity);
  mode = Run_Timer;
 }
...
void counter() //this will start counting down from the time set using the encoder. This begins when the encoder button is pressed.
{
  seconds = constrain (seconds, -1, 60);
  ...
  if (minutes == 0 & seconds == 0)
  {
    ...
    lcd.setCursor(0, 3);
    lcd.print("0000 B00M"); //Send the hour and minutes to the display
    Serial.print("buzz");
    buzz();
    mode = Set_Timer;
  }

  if (digitalRead(SBoutPin)== HIGH) // Si on appuie sur start
  {
    delay(Bout_Sensitivity);
    mode = Pause_Timer;
  }
